...
contract Dwitter {
    ...
    mapping(uint256 => Dweet) private dweets; // mapping to get dweet from Id
    mapping(address => uint256[]) private userDweets; // Array to store dweets(Id) done by user
    // mapping(uint=>address[]) private dweetLikersList;
    mapping(uint256 => mapping(address => bool)) private dweetLikers; // Mapping to track who liked which dweet
    ...
    mapping(uint256 => uint256[]) private dweetComments; // Getting comments for a specific dweet
    ...
    modifier onlyDweetAuthor(uint256 id) {
        require(msg.sender == dweets[id].author, 'You are not Author!');
        _;
    }
    ...
    modifier onlyActiveDweet(uint256 id) {
        require(dweets[id].status == cdStatus.Active, 'Not a active dweet');
        _;
    }
    ...
    event logDweetCreated(
        address author,
        uint256 userid,
        uint256 dweetid,
        string hashtag
    );
    event logDweetDeleted(uint256 id, string hashtag);
    ...
    /// @param  _id Id of dweet
    function deleteDweet(uint256 _id)
        public
        onlyActiveDweet(_id)
        onlyAllowedUser(msg.sender)
        stopInEmergency
    ...
    function likeDweet(uint256 _id)
        public
        onlyAllowedUser(msg.sender)
        onlyActiveDweet(_id)
    {
    ...
    function getUserDweets()
        public
        view
        onlyAllowedUser(msg.sender)
        returns (uint256[] memory dweetList)
    {
    ...
    /// @return dweetList Array of dweet ids
    function getUserDweets(address _user)
        public
        view
        onlyAllowedUser(msg.sender)
        returns (uint256[] memory dweetList)
    ...
    function createComment(uint256 _dweetid, string memory _comment)
        public
        stopInEmergency
        onlyAllowedUser(msg.sender)
        onlyActiveDweet(_dweetid)
    {
    ...
    function getDweetComments(uint256 _id)
        public
        view
        onlyAllowedUser(msg.sender)
        onlyActiveDweet(_id)
     ...
     */
    uint256 public totalMaintainers = 0;
    uint256[] private dweetsReportedList; // List of ids of reported dweets
    uint256 private noOfReportsRequired = 1; // No of reports required to term a dweet reported and send for action
    uint256 public reportingstakePrice = 1936458778290500; // Stake amount to pay while reporting
    uint256 public reportingRewardPrice = 3872917556581000; // Reward of correct reporting

    mapping(address => bool) public isMaintainer;
    mapping(address => uint256) private maintainerId;
    mapping(uint256 => mapping(address => bool)) private dweetReporters; // Mapping to track who reported which dweet
    mapping(uint256 => reportAction) private actionOnDweet; // What is the ction on dweet done by maintainers
    mapping(address => uint256[]) public userReportList; // Ids of dweets reported by a user
    //mapping(address=>uint) private userRewards;
    mapping(address => uint256) private fakeReportingReward; // Reward a user get when somebody do a fake reporting against that user
    mapping(uint256 => mapping(address => userdweetReportingStatus))
        private claimedReward; //To check whether user has claimed reward for a particular reporting

    enum userdweetReportingStatus {
        NP,
        Reported,
        Claimed
    }
    ...
    modifier paidEnoughforReporter() {
        require(
            msg.value >= reportingstakePrice,
            'You have not paid enough for advertisement'
        );
        _;
    }
    ...
    modifier checkValueforReporter() {
        _;
        uint256 amountToRefund = msg.value.sub(reportingstakePrice);
    ...
    event logDweetReported(uint256 id, string hashtag);
    event logDweetBanned(uint256 id, string hashtag, uint256 maintainer); // To track how many dweets were banned to specific hashtag
    event logDweetFreed(uint256 id, string hashtag, uint256 maintainer); // To track how many dweets were banned to specific hashtag

    /// @notice Report a dweet
    /// @param _dweetId Id of the dweet to be reported
    function reportDweet(uint256 _dweetId)
        public
        payable
        onlyActiveDweet(_dweetId)
        onlyAllowedUser(msg.sender)
        paidEnoughforReporter
        checkValueforReporter
    {
        require(
            dweets[_dweetId].reportCount <= noOfReportsRequired,
            'Dweet have got required no of Reports'
        );
        require(
            !dweetReporters[_dweetId][msg.sender],
        ...
        uint256 reports = dweets[_dweetId].reportCount;
    ...
    function takeAction(uint256 _dweetId, bool _action)
        public
        onlyMaintainer
        onlyActiveDweet(_dweetId)
        onlyAllowedUser(msg.sender)
    {
        require(
            actionOnDweet[_dweetId] == reportAction.NP,
        ...
        if (_action) {
            ...
            fakeReportingReward[dweets[_dweetId].author] = fakeReportingReward[
                dweets[_dweetId].author
            ].add(reportingstakePrice.mul(noOfReportsRequired));
            emit logDweetFreed(
                _dweetId,
                dweets[_dweetId].hashtag,
                maintainerId[msg.sender]
            );
        }
    }
    ...
    function claimReportingReward(uint256 _id)
        public
        onlyAllowedUser(msg.sender)
    {
        require(
            claimedReward[_id][msg.sender] == userdweetReportingStatus.Reported,
            'You have not reported or already claimed'
        );
        require(userReportList[msg.sender].length > 0);
        require(
            actionOnDweet[_id] == reportAction.Banned,
            'Not eligible for reward, Dweet has been freed my mainatiners'
        );
    ...
    function claimSuitReward() public onlyAllowedUser(msg.sender) {
        ...
        uint256 amount = fakeReportingReward[msg.sender];
    ...
    function reportingClaimStatus(uint256 _id)
        public
        view
        onlyAllowedUser(msg.sender)
        returns (userdweetReportingStatus status)
    {
    ...
    function fakeReportingSuitReward()
        public
        view
        onlyAllowedUser(msg.sender)
    ...
    function getReportedDweetStatus(uint256 _dweetId)
        public
        view
        onlyAllowedUser(msg.sender)
        returns (reportAction status)
    {
     ...
     *******************************************Advertisement **************************************************************
     */
    uint256 public advertisementCost = 96822938914524992;
    uint256 public totalAdvertisements = 0;
    uint256[] private advertisementsList;

    enum AdApprovalStatus {
        NP,
        Approved,
        Rejected
    }
    ...
    struct Advertisement {
        ...
        uint256 expiry; //timestamp to put expiry of Advertisement
    }

    modifier paidEnoughforAdvertisement() {
        require(msg.value >= advertisementCost);
        _;
    }
    modifier checkValueforAdvertisement() {
        _;
        uint256 amountToRefund = msg.value - advertisementCost;
        msg.sender.transfer(amountToRefund);
    }

    mapping(address => uint256[]) public advertiserAdvertisementsList;
    mapping(uint256 => Advertisement) private advertisements;

    event logAdvertisementApproved(uint256 id, uint256 maintainer);
    event logAdvertisementRejected(uint256 id, uint256 maintainer);
    ...
    function submitAdvertisement(string memory _imgHash, string memory _link)
        public
        payable
        onlyAllowedUser(msg.sender)
        paidEnoughforAdvertisement
        checkValueforAdvertisement
    {
        totalAdvertisements = totalAdvertisements.add(1);
        uint256 id = totalAdvertisements;
    ...
    function advertisementApproval(uint256 _id, bool _decision)
        public
        onlyMaintainer
    {
        require(
            advertisements[_id].status == AdApprovalStatus.NP,
        ...
        if (_decision) {
            ...
            uint256 refund = advertisementCost.mul(8).div(100);
    ...
    {
        return (
            advertisements[_id].advertiser,
            advertisements[_id].imgHash,
            advertisements[_id].link,
            advertisements[_id].status,
            advertisements[_id].expiry
        );
    }
    ...
    function getAdvertisementStatus(uint256 _id)
        public
        view
        onlyAllowedUser(msg.sender)
        returns (AdApprovalStatus status)
    {
